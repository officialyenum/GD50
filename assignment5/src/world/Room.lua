...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            -- drop health and make it random
            local randomHeart = math.random(4)
            local xHeart = entity.x
            local yHeart = entity.y
            entity.dead = true

            -- check if entity has a heart
            if entity.hasHeart == false then
                -- if it doesn't randomly generate a heart
                if randomHeart == 1 then
                    -- create heart object in killed entity location
        ...
        for k, object in pairs(self.objects) do
            -- if object collides with entity but the object is not in throw mode redirect entity
            if entity:collides(object) and object.carryMode == false and object.throwMode == false and object.solid == true then
                ...
                elseif entity.direction == 'up' then
                    ...
                    entity.y = entity.x + PLAYER_WALK_SPEED * dt
                    if entity.y + entity.height >= object.y then
                        entity.y = object.y - object.height - 1
                    end
                end

            -- if object collides with entity and the object is in throw mode destroy entity
            elseif entity:collides(object) and object.throwMode == true then
                entity:damage(1)
                gSounds['hit-enemy']:play()
                object.throwMode = false
                object.destroy = true
            end
        end
    end
    ...
    for k, object in pairs(self.objects) do
        ...
        if self.player:collides(object) then
            ...
            elseif object.solid == true and not object.throwMode then
