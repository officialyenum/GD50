...
function Level:init()
    ...
    -- initialize alien can split to true
    self.canSplit = true
    ...
    -- aliens in our scene
    self.aliens = {}
    -- new Aliens table for when space is pressed after launch
    self.newAliens = {}
...
function Level:update(dt)
    ...
    -- check if canSplit is still true
    if self.launchMarker.launched and love.keyboard.wasPressed('space') and self.canSplit then
        local xPos, yPos = self.launchMarker.alien.body:getPosition()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()

        -- create 2 new aliens top and bottom of launched alien
        local topAlien = Alien(self.world, 'round', xPos, yPos, 'Player') 
        topAlien.body:setLinearVelocity(xVel, yVel + 45)
        table.insert(self.newAliens, topAlien)

        local bottomAlien = Alien(self.world, 'round', xPos, yPos, 'Player') 
        bottomAlien.body:setLinearVelocity(xVel, yVel - 45)
        table.insert(self.newAliens, bottomAlien)

        -- disable split
        self.canSplit = false
    end
    for i = #self.newAliens, 1, -1 do
        local xPos, yPos = self.newAliens[i].body:getPosition()
        local xVel, yVel = self.newAliens[i].body:getLinearVelocity()
        -- if we fired our alien to the left or it's almost done rolling, respawn
        if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
            self.newAliens[i].body:destroy()
            table.remove(self.newAliens, i)
        end
    end
...
function Level:render()
    ...
    for k, alien in pairs(self.newAliens) do
        alien:render()
    end
